...
from ImageDataset2 import ImageDataset2, ImageDataset_qonly, ImageDatasetWithSoftLabels, ImageDatasetWithSoftLabels1, synImageDatasetWithSoftLabels, ImageFLIVE
from ImageDataset import ImageDataset_SPAQ, ImageDataset_TID, ImageDataset_PIPAL, ImageDataset_ava, ImageDataset_SPAQWithSoftLabels
...
def set_dataset1(csv_file, bs, data_set, num_workers, preprocess, num_patch, test, soft_labels_csv=None):
    """设置数据集，支持软标签"""

    if soft_labels_csv:
        # 使用带软标签的数据集
        data = ImageDatasetWithSoftLabels1(
            csv_file=csv_file,
            img_dir=data_set,
            num_patch=num_patch,
            test=test,
            preprocess=preprocess,
            soft_labels_csv=soft_labels_csv)
    else:
        # 使用普通数据集
        data = ImageDataset2(
            csv_file=csv_file,
...
def set_FLIVE(csv_file, bs, data_set, num_workers, preprocess, num_patch, test):
...
def set_syn_dataset(csv_file, bs, data_set, num_workers, preprocess, num_patch, test, soft_labels_csv=None):
    """设置数据集，支持软标签"""

    if soft_labels_csv:
        # 使用带软标签的数据集
        data = synImageDatasetWithSoftLabels(
            csv_file=csv_file,
            img_dir=data_set,
            num_patch=num_patch,
            test=test,
...
def set_spaq1(csv_file, bs, data_set, num_workers, preprocess, num_patch, test, soft_labels_csv=None):

    data = ImageDataset_SPAQWithSoftLabels(
        csv_file=csv_file,
        img_dir=data_set,
        num_patch=num_patch,
        test=test,
